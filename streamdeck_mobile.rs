...
fn client_key_id(key: &[u8]) -> String {
    // VSD2 verification code: first 6 hexadecimal characters of a
    // BLAKE2b hash with a 32-byte (256-bit) digest of the client public key.
    let mut h = Blake2bVar::new(32).expect("valid BLAKE2b-256 output size");
    h.update(key);
    let mut digest = [0u8; 32];
    h.finalize_variable(&mut digest)
        .expect("valid BLAKE2b-256 output buffer");
    hex6(&digest)
}
...
async fn handle_client(stream: TcpStream, peer: SocketAddr) -> anyhow::Result<()> {
	...
	log::info!("[VSD2] pairing workstation_id={} peer={peer}", workstation_code());
		...
		match message? {
			WsMessage::Binary(payload) => {
				...
				let client = match ClientMessage::decode(payload.as_ref()) {
					Ok(client) => client,
					Err(error) => {
						log::warn!("[VSD2] non-protobuf binary from {peer}: {} bytes: {error}", payload.len());
				...
				match client.payload {
					Some(client_message::Payload::Hello(hello)) => {
						if let Some(info) = hello.client_information.as_ref() {
							log::info!(
						...
						log::info!("[VSD2] Mobile public-key fingerprint for {peer}: {key_id}");
							...
							pending.insert(
								...
								PendingPairing {
									...
									name: hello
										...
										.map(|i| i.name.clone())
										.filter(|s| !s.is_empty())
										.unwrap_or_else(|| "Stream Deck Mobile".to_owned()),
									peer: peer.to_string(),
									approved: false,
								},
							);
						...
						log::info!("[VSD2] waiting for desktop confirmation code {} for {peer}", key_id);

						// Wait for the user to approve/reject the six-character
						// fingerprint shown by Stream Deck Mobile. This is deliberately
						// bounded so stale Mobile discovery connections do not pile up.
						let decision = tokio::time::timeout(Duration::from_secs(60), async {
							loop {
								if let Some(pairing) = PENDING_PAIRINGS.read().await.get(&key_id).cloned() {
									if pairing.approved {
										return Some(true);
									}
								} else {
						...
						if !decision {
							log::info!("[VSD2] desktop pairing rejected/timed out for {key_id} from {peer}");
							log::info!("[VSD2] pairing rejected or timed out; closing {peer}");
							return Ok(());
							return Ok(());
						}

						log::info!("[VSD2] desktop pairing approved for {key_id} from {peer}");
						...
						let server_hello = HelloFromServer {
							...
							// QR pairing is an authentication flow.
							client_needs_authentication: true,
						...
						log::info!("[VSD2] HelloFromServer sent to {peer}");
					}

					Some(client_message::Payload::Authenticate(auth)) => {
						log::info!("[VSD2] Authenticate from {peer}: request_id={}", auth.request_id);
						authenticated = true;

						let response = ok(auth.request_id);
						ws_tx.send(WsMessage::Binary(response.encode_to_vec().into())).await?;
						log::info!("[VSD2] authentication accepted for {peer}");
					}
					...
					Some(client_message::Payload::CreateVirtualDevice(create)) => {
						...
						let opened = VirtualDeviceOpened {
						...
						ws_tx
							.send(WsMessage::Binary(
								ServerMessage {
									payload: Some(server_message::Payload::VirtualDeviceOpened(opened)),
								}
								.encode_to_vec()
								.into(),
							))
					...
					Some(client_message::Payload::UpdateKeypadConfig(update)) => {
						...
						if let Some(id) = device_id.as_ref() {
							if let (Some(size), Some(device)) = (update.layout_crop.as_ref(), crate::shared::DEVICES.get(id)) {
								let rows = size.height.clamp(1, 255) as u8;
								let columns = size.width.clamp(1, 255) as u8;
								if device.rows != rows || device.columns != columns {
									log::info!("[VSD2] layout changed for {id}: {}x{} -> {}x{}", device.rows, device.columns, rows, columns);
								}
							}
						}
					}
					...
					Some(client_message::Payload::KeyPress(key)) => {
						if let Some(id) = device_id.as_ref() {
							...
							// Device registration is now real; the existing
							// OpenDeck event pipeline can observe this device.
							// Action dispatch remains owned by the normal
							// OpenDeck device/input machinery.
						}
					}
...
async fn run_mdns_announcer() {
	...
	let instance = format!("OpenDeck {}", hostname_string());
	let host = format!("opendeck-{}.local.", hex6(server_key()));
	let service = format!("{}.{}", dns_escape_name(&instance), SERVICE_TYPE);
...
fn dns_escape_name(name: &str) -> String {
	name.replace('.', "\\.")
}
...
fn build_mdns_response(service: &str, host: &str, instance: &str, ip: Ipv4Addr, port: u16) -> Vec<u8> {
	...
	out.extend_from_slice(&1u16.to_be_bytes());
	out.extend_from_slice(&120u32.to_be_bytes());
	let txt = format!("name={}", instance);
	let txt_len = 1 + txt.len();
