...
fn device_namespace(id: &str) -> Option<&str> {
...
async fn authorized_namespace(uuid: &str, id: &str) -> bool {
	if uuid.is_empty() {
		return true;
	}

	let Some(namespace) = device_namespace(id) else {
		return false;
	};
...
pub async fn register_device(uuid: &str, mut event: PayloadEvent<crate::shared::DeviceInfo>) -> Result<(), anyhow::Error> {
	...
	log::info!(
		"Registered plugin device id={} plugin={} rows={} columns={} encoders={}",
		event.payload.id,
		uuid,
		event.payload.rows,
...
/// Update plugin-device metadata without tearing down the device/profile lifecycle.
pub async fn update_device(uuid: &str, event: PayloadEvent<crate::shared::DeviceInfo>) -> Result<(), anyhow::Error> {
	...
	if changed {
		...
		if let Ok(profiles) = get_device_profiles(&id) {
			...
			for profile in profiles {
				if let Err(e) = profile_stores.get_profile_store_mut(&device, &profile).await {
					log::error!("Failed to refresh profile store for {}: {e}", id);
				}
			}
		}
		...
		log::info!(
			"Updated plugin device id={} name={} layout={}x{} encoders={} touchpoints={} infobars={}",
			id,
			device.name,
			device.rows,
			device.columns,
...
pub async fn deregister_device(uuid: &str, event: PayloadEvent<String>) -> Result<(), anyhow::Error> {
	if !authorized_namespace(uuid, &event.payload).await {
		return Err(anyhow::anyhow!(
			"plugin {uuid} is not registered for device namespace {}",
			device_namespace(&event.payload).unwrap_or("<invalid>")
		));
	}
	...
	log::info!("Deregistered plugin device id={} plugin={}", event.payload, uuid);
	Ok(())
}
