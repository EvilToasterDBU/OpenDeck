<script lang="ts">
	...
	async function poll() {
		try {
			...
			pollError = `Pending pairing command failed: ${String(e)}`;
		}
	}
...
<Popup show={show || pending.length > 0} label="Add Stream Deck Mobile">
	...
	<div class="flex flex-col items-center p-4 min-w-[500px]">
		{#if pending.length > 0}
			{#each pending as pair}
				<div class="w-full max-w-[460px] rounded-xl border border-blue-500/40 bg-neutral-800 p-4">
					<div class="text-sm font-semibold text-neutral-100">Confirm Stream Deck Mobile</div>
					<div class="mt-1 text-xs text-neutral-400">
						Compare this code with the code shown on your phone.
					</div>
					<div class="mt-4 text-center font-mono text-4xl tracking-[0.35em] text-neutral-100 select-all">
						{pair.fingerprint.toUpperCase()}
					</div>
					<div class="mt-2 text-center text-xs text-neutral-500">{pair.name} · {pair.peer}</div>
					<div class="mt-4 flex gap-2 justify-end">
						<button
							class="px-3 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white"
							on:click={() => resolve(pair.fingerprint, false)}
						>
							Reject
						</button>
						<button
							class="px-3 py-2 rounded-lg bg-green-600 hover:bg-green-500 text-white"
							on:click={() => resolve(pair.fingerprint, true)}
						>
							Approve
						</button>
					</div>
		...
		{#if pollError}
			<div class="mb-3 w-full max-w-[460px] rounded-lg border border-red-700 bg-red-950/40 p-3 text-xs text-red-300 font-mono break-all">
				Pending pairing command failed: {pollError}
			</div>
		{/if}
		...
		{#if info?.qr_data_url}
			...
			{#if pollError}
				<div class="mt-4 w-full max-w-[460px] rounded-lg border border-red-700 bg-red-950/40 p-3 text-xs text-red-300 font-mono break-all">
					{pollError}
				</div>
			{/if}
